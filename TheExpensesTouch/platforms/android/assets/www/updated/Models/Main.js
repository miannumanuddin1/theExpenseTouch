...
function updatesync(txt) {
    ...
    var d = (parseInt(dt.getFullYear())) + "-" + (parseInt(dt.getMonth()) + 1) + "-" + dt.getDate()+" "+time;
    var time = dt.getHours() + ":" + dt.getMinutes() + ":" + dt.getSeconds();
    //+ " " + dt.getHours + ":" + dt. + ":" + dt.getSecond;
...
function InsertExchangeRateForSync(CurrencyTypeId, CurrencyCode, USD, GBP, EUR, CHF, JPY, CNY, BHD, RUB, DKK, AUD, SEK, CAD, KRW, MXN, SGD, AED, BRL, ARS, LastModified, Deleted, GUID) {

    db.transaction(function (txt) {
        var syncstat = "INSERT INTO ExchangeRate (CurrencyTypeId,CurrencyCode,USD,GBP,EUR,CHF,JPY,CNY,BHD,RUB,DKK,AUD,SEK,CAD,KRW,MXN,SGD,AED,BRL,ARS,LastModified,Deleted,GUID) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
        txt.executeSql(syncstat, [CurrencyTypeId, CurrencyCode, USD, GBP, EUR, CHF, JPY, CNY, BHD, RUB, DKK, AUD, SEK, CAD, KRW, MXN, SGD, AED, BRL, ARS, LastModified, Deleted, GUID], function (txt, results) {
...
//alert(GUID);
    db.transaction(function (txt) {
        var Stat = "SELECT * FROM UserExpenses where GUID=?"
        txt.executeSql(Stat, [GUID], function (txt, result) {
         // alert(result.row)
            if (result.rows.length > 0) {
               // alert(result.rows.item(0).Deleted);
                if (result.rows.item(0).Deleted == "false") {
                 //   alert("Not Deleted");
                    ...
                    txt.executeSql(Syncstat, [Date, Description, Amount, ExpenseTypeId, ExpenseDocument, Duration, CurencyType, AmountToReimburse, LastModified, Deleted, GUID], function (txt, results) {
                        ...
                        if (results.rowsAffected == 0) {
                            //                      alert(Duration);
                            //          GetTravelId(Duration,function(txt,tid){
                            //                      alert("tid=="+tid);
...
function GetTravelId(TravelGUID,callback){
    db.transaction(function (txt) {
                   var Syncstat = "Select TravelId  from Travels where [GUID] =?";
                   txt.executeSql(Syncstat, [TravelGUID], function (txt, results) {
                                console.log(results);
                                  if(results.rows.length>0){
                               //   alert(results.rows.item(0).TravelID);
                                  callback(txt,results.rows.item(0).TravelID);
...
/*
function UpdateuserExpenseId(){
    db.transaction(function (txt) {
                   var Syncstat = "update userExpenses set travelid=  (Select TravelID  from Travels where [GUID] =userExpenses.[TravelGUID])";
...
function insertTravelForSyc(TravelId, UserId, Name, DateFrom, DateTo, Description, Status, StatusDate, ApprovalDate, RejectedDate, Comments, SubmittedDate, LastModified, Deleted, GUID) {
    db.transaction(function (txt) {
        ...
        var Syncstat = "Update Travels set TravelId=?,Name=?,DateFrom=?,DateTo=?,Description=?,Status=?,StatusDate=?,ApprovalDate=?,RejectedDate=?,Comments=?,SubmittedDate=?,LastModified=?,Deleted=? WHERE GUID=?";
        ...
        txt.executeSql(Syncstat, [TravelId,Name, getTDateOnly(DateFrom), getTDateOnly(DateTo), Description, Status, getTDateOnly(StatusDate), getTDateOnly(ApprovalDate), getTDateOnly(RejectedDate), Comments, getTDateOnly(SubmittedDate), LastModified, Deleted, GUID], function (txt, results) {
            if (results.rowsAffected == 0) {
        ...
        txt.executeSql(Syncstat, [], function (txt, results) {
            ...
            if (lenth > 0) {
                ...
                for (var k = 0; k < results.rows.length; k++) {
                    ...
                    jsonstr += '{ ExpenseId:' + row.ExpenseId + ', TravelId:' + row.TravelId + ', Date:' + getDateOnly(row.Date) + ', Description:' + row.Description + ', Amount:' + row.Amount + ', ExpenseTypeId:' + row.ExpenseTypeId + ',ExpenseDocument:' + row.ExpenseDocument + ', Duration:' + row.Duration + ', CurencyType:' + row.CurencyType + ', AmountToReimburse:' + row.AmountToReimburse + ', LastModified:' + row.LastModified.replace(":", "|").replace(":", "|").replace(":", "|").replace(":", "|") + ', GUID:' + row.GUID + ', Deleted:' + row.Deleted + ',TravelGUID:' + row.TravelGUID + '}';
                    if (k != lenth - 1) {
                        jsonstr+= ",";
                    }
                }
               ...
               // alert(new Date().getTime() - startTime);
                     //       SyncExchangeRates();
                          //  SyncUsers();
                            ...
                            updatelastsync();
             //  alert("Sync Successfully");   "Sync Successfully"
            },
            failure: function (errMsg) {
                //         alert(errMsg.data);
                console.log(errMsg); //errorMessage is id of the div
...
//alert(SyncTime);
    ...
    $.ajax({
       ...
       //     alert(5);
        ...
        //    app.hideLoading(); //hide loading animation
           navigator.notification.alert("Sync Successfully...",alertDismissed,'The Expense Touch','Done');
        //   UpdateuserExpenseId();
    ...
    $.ajax({
        ...
        success: function (response) {
            ...
            for (var i = 0; i < Kewords1.length; i++) {
                ...
                InsertExchangeRateForSync(Kewords1[i].CurrencyTypeId, Kewords1[i].CurrencyCode, Kewords1[i].USD, Kewords1[i].GBP, Kewords1[i].EUR, Kewords1[i].CHF, Kewords1[i].JPY, Kewords1[i].CNY, Kewords1[i].BHD, Kewords1[i].RUB, Kewords1[i].DKK, Kewords1[i].AUD, Kewords1[i].SEK, Kewords1[i].CAD, Kewords1[i].KRW, Kewords1[i].MXN, Kewords1[i].SGD, Kewords1[i].AED, Kewords1[i].BRL, Kewords1[i].ARS);
...
//    var options = { quality: 80,targetWidth: 500, targetHeight: 500, correctOrientation: true, destinationType: Camera.DestinationType.DATA_URL, sourceType: Camera.PictureSourceType.PHOTOLIBRARY, popoverOptions: popover };
...
//        //
//        cameraImage.src = "data:image/png;base64," + base64Img;
...
function successmanager(txt, results) {
  ...
  //      alert(localStorage["IsManager"]);
   ...
   window.location = "#homelandingpage";
...
function gettravel(txs) {
    var userid = localStorage.getItem("UserId");
  //  alert(userid);
    var getviewmytravel = "SELECT TravelId,UserId,Name,DateFrom,DateTo,Description,Status  FROM Travels WHERE UserId =? AND ifnull(Deleted,'false') =? AND Status  IS NULL";
    console.log(txs);
  ...
  // var d = new Date().getFullYear() + "-" + (parseInt(new Date().getMonth()) + 1) + "-" + new Date().getDate();
   // alert("update");
    // var userid = localStorage.getItem("UserId");
                             ...
                             var d = (parseInt(dt.getFullYear())) + "-" + (parseInt(dt.getMonth()) + 1) + "-" + dt.getDate() + " " + time;
                       //  alert(d);
    var updateExpenseStatement = "UPDATE UserExpenses SET Date=?,Amount=?,ExpenseTypeId=?,CurencyType=?,TravelId=?,ExpenseDocument=?,LastModified=? WHERE ExpenseId=?";
    console.log(updateExpenseStatement);
    txs.executeSql(updateExpenseStatement, [date, amount, expensetypeid, currencytype, travelid, image,d, expepenseid], userexpensupdatesucess, errorHandler);
}
...
function GetAllTravelByUserIdQueryS(txn) {
 ...
 //   var query = "SELECT * FROM Travels WHERE UserId =? AND Status IS NOT NULL";
    var query = "SELECT TravelId,UserId,Name,DateFrom,DateTo,Description,StatusDate,Status,ifnull((SELECT sum(AmountToReimburse) from userExpenses where travelid = Travels.travelid and   ifnull(Deleted,'false')='false' ),0) as Amount FROM Travels WHERE travels.UserId=? AND Status IS NOT NULL";
        ...
        for (i = 0; i < lengt; i++) {
            ...
            if (lengt > 0) {
                ...
                ///    newrow += '<td> <a data-id=' + results.rows.item(i).Travelid + ' class="travelbuttons" href="#SubmissionExpensesOFTravel-layout?TID=' + results.rows.item(i).TravelId + '">View</a> </td></tr>';
                    newrow += '<td>' + (results.rows.item(i).Amount).toFixed(2) + '</td>';
...
function GetAllTravelByUserIdForHomePage() {
    db.transaction(function (txa) { GetAllTravelByUserIdQueryHome(txa); }, errorHandler, successCB);
}
function GetAllTravelByUserIdQueryHome(txc) {
    var userid = window.localStorage.getItem("UserId");
    // var query = "SELECT TravelId,Date,Description,Amount,ExpenseTypeId,CurencyType FROM UserExpenses";
    var query = "SELECT TravelId,UserId,Name,Datefrom,DateTo,Description,Comments,SubmittedDate,Status FROM Travels WHERE UserId =?";
    console.log(query);
    console.log(userid);
    txc.executeSql(query, [userid], GetAllTravelByUserIdSSucessHomePage, errorHandler);
}
function GetAllTravelByUserIdSSucessHomePage(txt, results) {
    var lengt = results.rows.length;
    // alert("lentth " + lengt);
    ...
    if (lengt > 0) {
        ...
        for (i = 0; i < lengt; i++) {
            var travelStatus = results.rows.item(i).Status;
            if (travelStatus == null || travelStatus == "") {
                pending++;
            } else if (travelStatus == "Submited") {
                submitted++;
            } else if (travelStatus == "Appoved") {
                approved++;
